...
export class WalletService {
  ...
  static async connectApi(endpoint: string = ZKVERIFY_WS_ENDPOINT): Promise<ApiPromise> {
    // If a different endpoint is requested, disconnect from the old one first.
    if (this.api && this.api.isConnected && this.currentEndpoint !== endpoint) {
    ...
    if (!this.api) {
      try {
        ...
        this.currentEndpoint = endpoint; // Store the endpoint URL on successful connection
      } catch (error) {
